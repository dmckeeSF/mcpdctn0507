...
### Agent Metadata (Created at Runtime)
**Note**: GenAiFunction and GenAiPlugin metadata are NOT included in the package. These are runtime artifacts that Salesforce automatically generates when you add the Topic in Agent Builder. The packaged Apex actions will be discovered and wrapped by the platform.
...
### Why GenAiFunction/GenAiPlugin are NOT Packaged

GenAiFunction and GenAiPlugin metadata are **runtime artifacts** generated by Salesforce when users add actions in Agent Builder. They cannot be packaged because:
1. They reference the subscriber org's agent configuration
2. They're tied to specific agent instances
3. Salesforce automatically discovers and wraps `@InvocableMethod` Apex actions

When subscribers add your packaged actions (`d26__GetOpportunityFieldsAction`, `d26__CreateCustomObjectAction`) to their agents in Agent Builder, Salesforce automatically creates the GenAiFunction/GenAiPlugin wrappers.
